...
void Shutdown()
{
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateWorldGreenCoins(false, NULL);
    StopNode();
    {
        LOCK(cs_main);
        if (pwalletMain)
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("WorldGreenCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  worldgreencoind [options]                     " + "\n" +
                  "  worldgreencoind [options] <command> [params]  " + _("Send command to -server or worldgreencoind") + "\n" +
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "worldgreencoin:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: worldgreencoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: worldgreencoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 15844 or testnet: 16844)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 15843 or testnet: 16843)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. WorldGreenCoin is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("WorldGreenCoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (pwalletMain)
        GenerateWorldGreenCoins(GetBoolArg("-staking", true), pwalletMain);
